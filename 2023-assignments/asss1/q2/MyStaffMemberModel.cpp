...
QVariant MyStaffMemberModel::data(const QModelIndex &index, int role) const {
    ...
    MyStaffMember staffMember = staffMembers.at(index.row());
    if (role == Qt::DisplayRole) {

        QString n=staffMember.getNameSlot();
        QDate d=staffMember.getBirthdateSlot();
        QString a=staffMember.getAppointmentTypeSlot();

//        return QString("%1, %2, %3").arg(n,d,a);
        return QString("%1,%2,%3").arg(n, d.toString(),a);
    }
