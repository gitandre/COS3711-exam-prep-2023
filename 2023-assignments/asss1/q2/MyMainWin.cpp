...
MyMainWin::MyMainWin(QWidget *p)
    ...
    // popuate the drop down with values from Enum using reflection
    ...
    const int len = 3;
    std::array<QString, len> apptypeArray = matw->getAppTypeAsArray();
    for (int i = 0; i < len; i++) {
        QString val = apptypeArray[i];
        addQComboBox->addItem(val);
    }
...
void MyMainWin::SetupLayout() {
    ...
    subLayoutV->addWidget(new QLabel("© 2023 Bobby Nooba"));
...
//     validation checking
    if (!nameField->text().isEmpty() && nameField->text().size() >= 3) {
        ...
        if (age >= 18) {
            ...
            MyStaffMember msm;
            msm.setNameSlot(nameField->text());
            msm.setBirthdateSlot(dateField->date());
            msm.setAppointmentTypeSlot(addQComboBox->currentText());
            mySMModel->addStaffMember(msm);
